...
API_KEY = config.SECRET_KEY_WEATHER
AUTH_TOKEN = config.AUTH_TOKEN
DB_PASSWORD = config.DB_PASSWORD
OCG_API_KEY = config.OCG_API_KEY
...
def send_sms(phone_number, message_body):
    ...
    try:
        query = "SELECT user_phone FROM User_Items"
        ...
        try:
            ...
            body=message_body,
            from_='+13158186457',  
            to=phone_number  # Extract the phone number from the fetched result
            )
            return f'("Message sent to:", phone_number, "Status:", message.status)'
...
def send_weather_notifications(user_id):
    try:
        # Connect to the PostgreSQL database and retrieve latitude and longitude for each user
        query = f"SELECT latitude, longitude, user_phone FROM User_Items WHERE user_id = '{user_id}' "
        users = manage_connection(query)
        # print(users) #test
        # input("test")
        # for user in users:
        latitude, longitude, phone_number = users[0]
